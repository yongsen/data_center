...
\begin{frame}{Failures handling and Multi-path TCP}{Basic Procedures}
\begin{enumerate}
  \item Split flow(s) into subflows
  \item Congestion control by block ACK
  \item Add premix to subflows
  \item Transmit subflows
  \item Decoding according to premix and delay (failures occur)
\end{enumerate}
\end{frame}

\begin{frame}{Failures handling and Multi-path TCP}{Adopted Technologies}
...
\begin{itemize}
  ...
  \item \textbf{Flexible switching} improves the capacity and reduces the delay: 10G $\rightarrow$ 20G, decoding after transmit $\rightarrow$ as transmitting.
\end{itemize}
\end{block}
